...
Dao is a web and desktop GUI for running coding agents on your machine.

## Requirements

Node.js `^22.16 || ^23.11 || >=24.10` on the machine that runs the Dao server.

At least one provider runtime, installed and authenticated. You can install Antigravity from
Dao settings. See [Providers](#providers) below.
...
This starts the Dao server on your machine and opens the local web app. Use
`npx t3@latest --help` for the full CLI reference.

## Open a project in the desktop app

When the Dao desktop app is running on the same machine, open the current directory with:
...
thread. It does not launch the desktop app, open a browser, or start a Dao server. A background
...
`~/.t3/wsl-runtime` inside the selected distro. The first launch after installing or updating Dao
...
successful launch, Dao keeps the current runtime and one previous runtime for rollback and
removes older caches automatically. If a cached runtime stops working, Dao launches from the
application files under `/mnt/c` instead and reinstalls the runtime on the next launch.

## Providers

Dao uses provider runtimes but does not bundle them. Install and authenticate each
provider's CLI, or use Dao's managed setup for Antigravity.
...
| Antigravity | [Official ACP agent](https://github.com/agentclientprotocol/registry/blob/main/antigravity-acp/agent.json) | Managed by Dao | **Sign in with Google** in Dao |
...
Dao looks for, but authenticate with `agent login`, not `cursor-agent login`.
...
Run CLI login commands on the machine running the Dao server, not on the device you browse
from. Antigravity uses its sign-in controls in Dao instead of a CLI login command.
...
Dao. You can install Dao, open it, and add providers afterwards. A provider that is not
...
- [Permission modes](./permission-modes.md): how much Dao asks before acting
- [Remote access](./remote-access.md): connect from a phone, tablet, or another desktop
- [Keeping Dao in sync](./updating.md): client and server version skew
