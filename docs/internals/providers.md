...
A provider is the agent runtime that does the actual work. Dao supports several, and the
...
Dao does not own an external OpenCode process. Native configuration changes there can require
an external reload or restart before Dao's next refresh sees them.
