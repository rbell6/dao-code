...
async function seedDaoWorkspace(workspaceRoot: string): Promise<void> {
...
async function seedCompanionWorkspace(input: {
  ...
  await NodeFSP.writeFile(
    ...
    `# ${input.title}\n\nSeeded by the Dao mobile screenshot harness.\n`,
  );
  await initializeRepository({
...
export async function seedShowcaseEnvironment(input: {
  ...
  if (primaryProject.id === SHOWCASE_PROJECT_ID) {
    await seedDaoWorkspace(workspaceRoot);
  }
  await Promise.all(
    projects
